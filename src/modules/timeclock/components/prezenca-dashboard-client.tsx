...
const NAV_BTN =
  "inline-flex h-8 w-8 items-center justify-center rounded-lg border border-line bg-white text-ink-700 transition-colors hover:bg-fill-hover";
const MICRO = "text-[10.5px] font-bold uppercase tracking-[0.04em] text-ink-400";
    ...
    <div className="space-y-6">
      ...
      <div className={`flex flex-wrap items-center justify-between gap-x-6 gap-y-3 px-4 py-3 ${CARD}`}>
        <div className="flex items-center gap-2">
          ...
          <h2 className="min-w-[128px] text-center text-[13.5px] font-bold capitalize tracking-[-0.01em] text-ink-900">
            {monthLabel}
          </h2>
        ...
        <dl className="flex items-stretch divide-x divide-line-soft">
          <div className="px-4 first:pl-0">
            <dt className={MICRO}>Punonjës me skanime</dt>
            <dd className="mt-0.5 text-[19px] font-extrabold leading-none tabular-nums text-ink-900">
          ...
          <div className="px-4 last:pr-0">
            ...
            <dd
              className={`mt-0.5 text-[19px] font-extrabold leading-none tabular-nums ${
                presence.totals.reviewDays > 0 ? "text-[#b45309]" : "text-ink-900"
              }`}
            >
              ...
              return (
                <li key={key} className="flex flex-wrap items-center gap-3 px-5 py-3">
                  ...
                  <button
                    ...
                    className="inline-flex h-8 items-center rounded-lg border border-line bg-white px-3 text-[12.5px] font-semibold text-ink-700 transition-colors hover:bg-fill-hover disabled:opacity-60"
                  >
      ...
      <div className={`overflow-hidden ${CARD}`}>
        {presence.employees.length === 0 ? (
          <div className="px-5 py-12 text-center">
            <AlarmClock className="mx-auto h-8 w-8 text-ink-300" aria-hidden />
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[940px]">
              {/* day header */}
              <div
                className="grid h-9 border-b border-line-soft"
                style={{ gridTemplateColumns: `220px repeat(${daysInMonth}, minmax(24px, 1fr)) 88px` }}
              >
                <div className="sticky left-0 z-[3] flex items-center border-r border-line-soft bg-white px-3">
                ...
                {dayNumbers.map((d) => (
                  <div
                    ...
                    className={`flex items-center justify-center text-[10.5px] font-semibold tabular-nums ${
                      isWeekendCol(d) ? "bg-[#fbfcfe] text-ink-300" : "text-ink-400"
                    } ${isoOf(d) === props.todayIso ? "shadow-[inset_2px_0_0_#2563EB]" : ""}`}
                  >
                    {d}
                  </div>
                ))}
                <div className="flex items-center justify-end border-l border-line-soft pr-3">
              ...
              {presence.employees.map((emp) => {
                ...
                return (
                  ...
                  >
                    <div className="sticky left-0 z-[3] flex min-w-0 flex-col justify-center border-r border-line-soft bg-white px-3">
                      <Link
                        href={`/punonjesit/${emp.employeeId}`}
                        className="truncate text-[12.5px] font-semibold text-ink-900 hover:underline"
                      >
                        {emp.name}
                      </Link>
                      {emp.flags.length > 0 ? (
                    ...
                    <div className="flex flex-col items-end justify-center border-l border-line-soft pr-3">
        ...
        {/* legend */}
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 border-t border-line-soft px-4 py-2.5 text-[11px] font-medium text-ink-500">
