...
export function EmployeeStatusBadge({
  ...
  if (status === "INACTIVE") {
    return <Badge variant="warning">{EMPLOYMENT_STATUS_LABELS.INACTIVE}</Badge>;
  }
  if (status === "ON_LEAVE") {
    return <Badge variant="secondary">{EMPLOYMENT_STATUS_LABELS.ON_LEAVE}</Badge>;
  }
  if (status === "SUSPENDED") {
    return <Badge variant="warning">{EMPLOYMENT_STATUS_LABELS.SUSPENDED}</Badge>;
  }
  if (employmentType === "CONTRACTOR") {
    return <Badge variant="outline">{EMPLOYMENT_TYPE_LABELS.CONTRACTOR}</Badge>;
  }
